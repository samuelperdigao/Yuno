...
def delivery_out(item) -> DeliveryWorkItemOut:
    return DeliveryWorkItemOut(
        id=item.id,
...
@router.post(
    "/guilds/{guild_id}/modules/{module_key}/deliveries",
    response_model=DeliveryWorkItemOut,
)
async def enqueue(
    guild_id: str,
    module_key: str,
...
@router.post("/deliveries/claim", response_model=list[DeliveryWorkItemOut])
async def claim(
    data: WorkClaimIn, session: AsyncSession = Depends(get_session)
) -> list[DeliveryWorkItemOut]:
    return [delivery_out(item) for item in await service.claim_deliveries(session, **data.model_dump())]


@router.post(
    "/guilds/{guild_id}/deliveries/{delivery_id}/complete",
    response_model=DeliveryWorkItemOut,
)
async def complete(
    guild_id: str,
...
@router.post(
    "/guilds/{guild_id}/deliveries/{delivery_id}/fail",
    response_model=DeliveryWorkItemOut,
)
async def fail(
    guild_id: str,
