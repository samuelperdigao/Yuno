...
def test_meta_migration_on_empty_sqlite(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(database) as connection:
        ...
        assert "chest_movements" in tables
        assert "chest_balances" in tables
...
def test_representative_migration_archives_then_removes_legacy_farm_domains(
    ...
    with sqlite3.connect(database) as connection:
        connection.execute(
            ...
            (
                ...
                json.dumps(
                    {"meta": {"channel_id": "9"}, "ticket": {"channel_id": "8"}}
                ),
            ),
        )
        connection.execute(
    ...
    with sqlite3.connect(database) as connection:
        ...
        assert (
            connection.execute(
                "SELECT COUNT(*) FROM module_config_states WHERE module_key = 'meta'"
            ).fetchone()[0]
            == 0
        )
        assert "farm_tickets" not in tables
