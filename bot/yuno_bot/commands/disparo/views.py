...
from yuno_bot.commands.disparo.embeds import log_exclusao_embed
from yuno_bot.commands.disparo.helpers import send_log
...
class DeleteDisparoConfirmView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.requester_id:
            await interaction.response.send_message(
                "Apenas quem iniciou a exclusão pode confirmar esta ação.", ephemeral=True
            )
            return False
        return True
    ...
    async def confirmar(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        try:
            await self.api.patch_record(
                module="disparo",
                record_id=self.batch_record_id,
                status="cancelled",
        ...
        try:
            ...
            await send_log(
                interaction.guild,
                config,
                log_exclusao_embed(autor=interaction.user, deletados=deletados, falhas=falhas),
            )
        except httpx.HTTPError:
            pass
...
class DisparoPanelView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="yuno:disparo:panel:enviar",
    )
    ...
    @discord.ui.button(
        label="Apagar Último Disparo",
        emoji="🗑️",
        style=discord.ButtonStyle.danger,
        custom_id="yuno:disparo:panel:apagar",
    )
    @requires_module("disparo", "enviar")
    async def apagar(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        await interaction.response.send_message(
            f"Você está prestes a apagar {count} mensagem(ns) do último disparo. "
