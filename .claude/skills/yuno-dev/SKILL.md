...
2. **`messages` é consumido pelos painéis fixos, mas não por todas as respostas e registros.** Título, descrição e cor dos painéis são configuráveis; textos de sucesso, erro e embeds de registros ainda têm fallback fixo no código.
3. **Não existe conceito de plano por licença.** `plano_minimo` é declarado em todo `ModuleSpec` (basico/pro/premium) mas não tem ligação com nenhum dado de `License`/`GuildConfig` — hoje todo módulo liga por padrão pra todo mundo. Bloqueia diferenciação de preço por módulo.
...
- Painel geral dentro do Discord (`bot/yuno_bot/dashboard.py`, comando `/yuno painel`). Mostra estado, ativa/desativa módulos e aponta o comando especializado de configuração/publicação. Campos com efeitos colaterais continuam nos comandos nativos (`/set painel`, `/meta painel`, etc.) para não duplicar regras frágeis. `dashboard_fields` foi auditado contra o código real e `/setup_farm_tickets` passou a exigir `participant_role_ids`.
- Acabamento comercial dos painéis (2026-08-02): painel geral paginado para respeitar Components V2, ativação/desativação de módulos dentro do Discord, publicador idempotente compartilhado (`commands/panels.py`), painéis novos de ticket/encomenda/produção/advertência, referências persistidas e personalização por `messages.<modulo>.panel`.
- Permissões comerciais: `radio`, `ticket`, `encomenda`, `producao` e `adv` aceitam cargos configuráveis nos comandos de painel. `/radio alterar` deixou de depender do nome do cargo e todos os comandos/botões operacionais passam por validação de licença/módulo/permissão no backend.
- Farm Fase 3 (escopo aprovado): `/farm ranking` agregado no backend, sem quota obrigatória ou punição automática. O painel de farm ganhou botão persistente de ranking e deixou de duplicar mensagens ao republicar.
- Visibilidade do SET: `_apply_set_visibility` preserva overwrites existentes e só chama a API para categorias/canais cujo estado realmente precisa mudar; canais filhos que já herdam o bloqueio deixam de gerar uma chamada por setup.
